...
export class DragStateManager {
    constructor(scene, renderer, camera, container, controls) {
        ...
        this.arrow = new THREE.ArrowHelper(new THREE.Vector3(0, 1, 0), new THREE.Vector3(0, 0, 0), 20, 0x666666);
        ...
        this.arrow.line.material.opacity = 0.9;
        this.arrow.cone.material.opacity = 0.9;
    ...
    update() {
        if (this.worldHit && this.localHit && this.currentWorld && this.arrow && this.physicsObject) {
            ...
            const diff = this.currentWorld.clone().sub(this.worldHit);
            const len = diff.length();
            this.arrow.setDirection(diff.normalize());
            // Even smaller head
            const headLen = Math.max(0.1, Math.min(0.6, len * 0.12));
            const headWidth = Math.max(0.06, Math.min(0.3, len * 0.08));
            this.arrow.setLength(len, headLen, headWidth);
        }
    }
    end(evt) {
